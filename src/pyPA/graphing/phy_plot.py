...
def chem_plot(job, nlines = 8, combine = [()], fmt = 'pdf'):
    mech = get_pure_mech('_'.join([job.mechanism.lower(),job.model.lower()]))
    tz = {'cmaq': 'UTC', 'camx': 'LST'}[job.model.lower()]
    ...
    for species in job.species:
        ...
        for rxn in reactions[:nlines-1]:
            ...
            options['label'] = re.compile('\d+.\d{5}\*').sub('', str(mech(rxn).sum()))
        ...
        try:
            ...
            data = mech.make_net_rxn(mech(species), mech(species), False).sum()[mech(species)]
        ...
        legend(loc=(0,1), prop = FontProperties(size=10))
...
if __name__ == '__main__':
    ...
    job.species  = dict(HNO3=dict(ymin=None),
                        NO2=dict(ymin=None),
                        CXO3=dict(ymin=None),
                        C2O3=dict(ymin=None))
    ...
    job.end_date = False
    phy_plot(job)
#    chem_plot(job, combine = [('RXN_01', 'RXN_02', 'RXN_03')])
    chem_plot(job)